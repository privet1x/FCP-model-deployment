...
# modified to be used in api
def train_model(data, model=None, scalar=None):
    x = data.loc[:, data.columns != 'auth']
    y = data.loc[:, data.columns == 'auth']

    if model is None or scalar is None:
        # If no model/scalar provided, create and train from scratch
        ...
        # Continue training the provided model
        x_scaled = scalar.transform(x)
        model.fit(x_scaled, y.values.ravel())
        return model, scalar


# Modified it to use in fastapi endpoint because function did not return data
